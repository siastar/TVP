...
class DBApp extends React.Component {
    ...
    addProd(newProduct) {
        axios.post(postDataRoute, newProduct)
            .then(res => {
                ...
                Object.keys(newProduct).forEach(function(key) { delete newProduct[key]; });
                Object.keys(handledProduct).forEach(function(key) { delete handledProduct[key]; });
    ...
    editProd(crudArgs) {
        ...
        axios.put(axiosRoute, crudArgs.handledProduct)
            .then(res => {
                //// STATE UPDATER
                ...
                let index = newProducts.findIndex(item => item._id === crudArgs.target_id);
                ...
                Object.keys(crudArgs).forEach((key) => { delete crudArgs[key]; });
                Object.keys(handledProduct).forEach((key) => { delete handledProduct[key]; });
    ...
    handleCRUDType(crudArgs) { //receives object data defined by triggerCRUDAction in SingleProd.js
        switch (crudArgs.crudAction) { // e.g. crudArgs = {_id: "5e39f88e23a5b1412830572c", crudAction:"CRUD_delete"}                                          
    ...
    render() {
        ...
        return ( <div className = "container" >
                     ...
                     /> {/*products showcase */ }
                   <ShowProdsList className = 'showprodslist'
                                  productHandlers = {productHandlers}
