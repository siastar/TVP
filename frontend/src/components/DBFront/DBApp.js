...
class DBApp extends React.Component {
    ...
    addProd(newProduct) {
        //this.testXYZ();
        //let justAddedProduct = '';
        console.log('addProd args...', newProduct);
        axios.post(postDataRoute, newProduct)
            .then(res => {
                //console.log('xxxx product added to DB successfully');
                //console.log('xxxx res', res.data.message)
                //now the problem is that newProduct does not contain the DB _id
                //because _id is generated by the DB itself after the axios.post
                ...
                //it happens that a copy of the just added product comes back automatically,
                //nested in the axios message response as "res.data.message.newdata._id", so we can grab it and push
                //in the state,the products array 
                let justAddedProduct = res.data.message.newdata;
                console.log('just added  product: xxxxx', justAddedProduct);
                //newProduct._id = newProduct_id
                //console.log('newProduct with _id returned from mongoDB: xxxxx', newProduct);
                ...
                console.log('newProducts xxxxx', newProducts);
                //update the state by replacing full products array
                this.setState({
                    products: newProducts
                });
                console.log('DBApp state: xxxxx', this.state);
                ////newProduct._id = null;
                ///
            })
            .catch(err => {
    ...
    editProd(crudArgs) {
        //console.log('zzz axios.put crudArgs: ', (crudArgs));
        ...
        axios.put(axiosRoute, crudArgs.handledProduct)
            .then(res => {
                ...
                //// STATE UPDATER
                //// extract from crudArgs.handledProduct the edited properties
                const targetProduct = res.data.message.updated
                //console.log('zzz targetProduct' ,  targetProduct)
                //// res.data.message.updated comes from the server after update
                //// and returns the object to update
                ...
                let newProducts = [...this.state.products];
                //console.log('zzz newProds' ,  newProducts);
                ...
                //console.log('zzz newProducts:' , newProducts);
    ...
    handleCRUDType(crudArgs) { //receives object data defined by triggerCRUDAction in SingleProd.js
        console.log('zzz handleCRUDType args: ', crudArgs); //the object looks like:                                                    
        switch (crudArgs.crudAction) { // e.g. crudArgs = {_id: "5e39f88e23a5b1412830572c", crudAction:"CRUD_delete"}                                          
            ...
            case 'CRUD_create': //receives object data defined by onFormSubmit in AddProd.js
                console.log('creating...yyyyy', crudArgs); //the object looks like:
    ...
    onEditSubmit(event, product) {
        //console.log('zzz prod', product);
        event.preventDefault();
        //console.log('zzz product:', product);
        //console.log('zzz handledProduct' , handledProduct);
      ...
      //console.log('zzz object to deliver, crudArgs:', crudArgs);
      this.handleCRUDType(crudArgs);
